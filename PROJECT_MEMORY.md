# PROJECT_MEMORY

## Modern 分支迁移结论

- 这个仓库历史上对 `torchlight` 的使用依赖“单独安装 `torchlight/` 子包”；在 modern 分支引入仓库根 `pyproject.toml` 后，若不补仓库根 [torchlight/__init__.py](/home/atk/PyCharm/ST-GCN/torchlight/__init__.py)，直接运行 `python main.py` 只会拿到 namespace package，`import torchlight` 不具备 `IO` / `import_class` 等接口。
- 当前 modern 分支已通过仓库根兼容层恢复两种入口：
  1. 直接在仓库根运行 `main.py`
  2. 作为包安装后导入 `torchlight`
- Python 版本约束不应写成宽泛的 `>=3.12`，否则会把未来未验证的 `3.14+` 自动纳入兼容面；当前项目已收敛为 `>=3.12,<3.13`。
- 在本节点上，`uv sync` 已成功创建 `.venv`，并实际使用 `CPython 3.12.12`。后续做 modern 分支验证时，优先直接使用 `.venv/bin/python`，不要继续用系统 `python3` 推断仓库状态。
- `uv run` 在当前沙箱里可能因 `~/.cache/uv` 不可写而失败；若 `.venv` 已经存在，优先直接调用 `.venv/bin/python` 做验证，避免把缓存权限问题误判成项目问题。
- 已做过的有效验证：
  - `.venv/bin/python main.py -h`
  - `.venv/bin/python main.py recognition -h`
  - 单流 `net.st_gcn.Model` 前向与 `extract_feature` 输出形状验证
  - 双流 `net.st_gcn_twostream.Model` 前向形状验证
- 与 `git show HEAD:net/st_gcn.py` / `git show HEAD:net/utils/graph.py` 的对照验证已经做过：
  - 图邻接矩阵完全一致
  - `eval()` 下单流前向输出完全一致
  - `eval()` 下 `extract_feature` 的输出与特征张量完全一致
  - 双流模型的实质性变化仅是把原版写死的 `torch.cuda.FloatTensor(...)` 改成了 device-agnostic 写法，数学公式未改

## 当前已知缺陷 / 未完成验证

- `demo_old`、`demo`、`demo_offline` 还没有在真实 OpenPose 环境下做端到端运行验证；当前只有代码级迁移、参数解析和导入级保证。
- `demo_offline` 在最初的 modern 重写里曾误引入两处与官方不一致的 tracker 行为：`get_skeleton_sequence()` 对 trace 按分数排序并截断到前 2 人，以及 `get_dis()` 使用了对角线尺度 `sqrt(w^2 + h^2)`。这两处现已回滚到官方语义；后续若再动 demo 逻辑，必须先明确这是兼容修复还是有意行为变更。
- `DemoOffline.pose_estimation()` 当前已改为在 OpenPose Python API 缺失时显式返回 `None`，`start()` 会抛出清晰异常而不是沿用官方版的隐式解包失败；这属于失败路径诊断改进，不影响成功路径。
- `Graph.__str__` 已从返回 `np.ndarray` 对象改为返回字符串，属于纯代码卫生修复，不影响图构造与模型计算。
- 与官方实现的严格数值等价性，目前只验证到了 `eval()` 路径；`train()` 态下出现数值不完全一致主要来自 dropout / BatchNorm 的训练时随机性与状态更新，不应直接解读为结构偏离，但也还没有做更细的逐层训练态对照。
- 当前还没有用官方预训练权重做一次完整的 `recognition --phase test` 回归，因此旧 checkpoint 的加载语义虽然保留，仍缺少一次真实数据集上的端到端结果确认。

## 2026-04-12 - Herald 与 tqdm 日志改造
- Context: 用户要求把训练时的 iter 刷屏改成 `tqdm` 进度条，并接入 `Herald`，终端隐藏 debug 级 iter 明细，但文件日志尽量保留原有完整输出。
- Decision: 在 `torchlight.IO` 内集中接入 `Herald`，使用 `ConsoleHandler(level="info") + FileHandler(level="debug")`；训练和测试循环单独使用 `tqdm` 展示 iter 进度，不让 iter 文本再走终端日志。
- Why: `Herald` 的 `Logger` 先做全局级别过滤，再由 Handler 做二次过滤；因此若要保留文件中的 debug iter 明细，logger 本身必须保持 `debug`，只把终端 Handler 收紧到 `info`。`tqdm` 不能视为普通 info 日志，否则会被级别策略误伤。
- Action/Command: `uv add tqdm 'Herald @ git+https://github.com/ArtichoKaleyard/Herald'`；`.venv/bin/python -m compileall main.py processor net feeder tools torchlight`；`.venv/bin/python main.py recognition -h`；用 `torchlight.IO('temp/herald_probe', save_log=True, print_log=True)` 做最小日志探针，确认控制台隐藏 debug、`log.txt` 保留 debug。
- Verification: 已确认 `Herald` 可正常导入；控制台只显示 `info/success`，文件日志包含 `debug`；CLI 帮助与语法检查通过。
- Follow-up: 后续若继续收敛噪声，可再评估是否把逐层权重加载明细长期保留在 `debug`，并用真实训练配置检查 `tqdm` 在长跑任务中的观感。

## 2026-04-12 - 两次正式训练结果可视化
- Context: 用户要求把同一数据集上的两轮正式训练结果做成学术排版对比图；当前目标是对比仓库默认配置与论文学习率日程配置，不手工计算子图坐标。
- Decision: 新增 [tools/plot_training_comparison.py](/home/atk/PyCharm/ST-GCN/tools/plot_training_comparison.py)，默认直接读取 `work_dir/recognition/ntu-xsub/ST_GCN_fp32_bs16_lr0025_e80` 与 `work_dir/recognition/ntu-xsub/ST_GCN_fp32_bs16_lr0025_e80_step10x`，统一抽取训练均值 loss、验证 Top-1/Top-5 与 epoch 级学习率，并用 `subplot_mosaic(..., constrained_layout=True)` 输出 2x2 对比图。
- Why: 当前仓库内两次正式训练的 `log.txt` 并非同一种格式。默认配置目录仍是旧版时间戳文本，论文配置目录则是 Herald 的多列日志；若只按一种格式写解析器，后续图脚本会立即失效。稳定锚点是 `Training epoch`、`mean_loss`、`Eval epoch`、`Top1/Top5` 与 iter 行中的 `lr`。
- Action/Command: 使用 `.venv/bin/python tools/plot_training_comparison.py` 实际渲染；首次运行因缺少 `matplotlib` 失败后，按项目约束执行 `uv add matplotlib` 补齐依赖；为避免 Matplotlib 在当前环境里因 `~/.config/matplotlib` 不可写而报缓存目录警告，脚本入口显式设置 `MPLCONFIGDIR=ROOT_DIR/temp/matplotlib`。
- Verification: `python3 -m compileall tools/plot_training_comparison.py` 通过；已成功导出 [ntu_xsub_training_comparison.png](/home/atk/PyCharm/ST-GCN/work_dir/figures/ntu_xsub_training_comparison.png) 与 [ntu_xsub_training_comparison.pdf](/home/atk/PyCharm/ST-GCN/work_dir/figures/ntu_xsub_training_comparison.pdf)。当前两次正式训练的最终验证指标为：默认配置 `Top1=78.75% / Top5=96.32%`，论文学习率日程 `Top1=79.90% / Top5=96.73%`。
- Follow-up: 这份脚本目前默认绑定 NTU xsub 的两组正式训练目录；如果后续要支持更多实验组合，优先扩展 `--run LABEL WORK_DIR` 输入，而不是在脚本内部继续写死新的目录分支。

## 2026-04-13 - 第一轮消融实验协议
- Context: 用户准备先做 NTU RGB+D xsub 的第一轮消融，只跑当前仓库已有实现底座的 4 项：`uniform_noimp`、`distance_noimp`、`spatial_noimp`、`spatial_imp`；目标是先看相对趋势，不追求一步到位复现论文最终绝对数值。
- Decision: 第一轮统一协议固定为 `40 epoch + eval_interval=5 + 单 seed + 单卡 batch_size=16 + base_lr=0.025 + step=[10,20,30] + AMP`，并把 4 份配置放到 `config/ablation/ntu-xsub/`，对应工作目录放到 `work_dir/ablation/ntu-xsub/`。
- Why: 当前正式训练曲线已确认 40 轮后收益很弱，继续拉长到 80 轮会显著增加时间成本；但直接缩到 30 轮又容易把“谁前期起步快”误当成结构结论。对 40 轮短程协议而言，把长程多段衰减显式裁成有效前缀 `step=[10,20,30]` 不改变可达 epoch 内的行为，只是去掉不可达的后续衰减点。AMP 在这轮实验里视为统一训练协议的一部分，而不是单独消融变量。
- Action/Command: 新增 4 份 ablation config，并在 `processor/recognition.py` 补充 `--amp` 开关，使用 `torch.amp.autocast` 与 `GradScaler` 支持 CUDA AMP 训练/评测。
- Verification: 后续至少应做 `recognition -h` 参数检查，以及逐份配置的 YAML 读取检查和基础语法检查；真正的趋势判断以 4 项实验全部跑完后的 Top-1 排序为准。
- Follow-up: 第一轮完成后，优先检查 `spatial_imp > spatial_noimp` 以及 `spatial` 系列相对 `uniform/distance` 的排序是否合理；`baseline_tcn` 与 `local_conv` 留到第二轮，不在本次配置里提前混入。

## 2026-04-13 - 阶段二消融预研
- Context: 用户要求先只跑第一轮 4 个现成消融，但希望提前预研第二轮大致该怎么做。
- Decision: 阶段二优先顺序定为 `baseline_tcn` -> `local_conv`。`baseline_tcn` 适合做成一个最小纯时域卷积模型，单独回答“去掉图结构会掉多少”；`local_conv` 暂不实现，只保留技术设计预研，因为它最容易在“局部连接不共享权重”的定义上做偏。
- Why: 当前仓库的 ST-GCN 主干和 `net/utils/tgcn.py` 已经把“共享卷积 + 邻接矩阵聚合”的结构拆清楚了，做 `baseline_tcn` 风险低、解释也直接；而 `local_conv` 的风险不在代码量，而在定义是否仍符合论文语义，以及参数量是否会失控。
- Action/Command: 预研结论是后续若进入阶段二，优先新增 `net/baseline_tcn.py` 与对应 ablation config；`local_conv` 则优先新增 `net/utils/local_tgcn.py` 设计稿，再决定是否补 `net/local_conv.py`，不要直接边写边改主模型。
- Verification: 当前只是方案预研，不涉及代码实现或数值验证。
- Follow-up: 真正进入阶段二前，先锁死 `local_conv` 的定义边界：是否沿用 `spatial` partition、参数是否按 node 独立、是否需要约束参数量，避免写成“另一个新模型”而不是论文消融项。

## 2026-04-13 - AMP 最短探针
- Context: 在正式串行跑 4 个第一轮消融之前，用户要求先做一个最短临时测试，对比 AMP 是否会在当前训练链路里引入明显异常。
- Decision: 基于 `config/ablation/ntu-xsub/spatial_imp.yaml`，各跑一组 `debug + 1 epoch + eval_interval=1` 的临时探针，工作目录分别为 `temp/amp_probe_off` 与 `temp/amp_probe_on`。
- Why: 用户只要求验证 AMP 是否“可放心作为统一实验协议的一部分”，而不是要做严格 AMP 消融；因此最小可比探针就够了，不需要额外延长训练。
- Action/Command: `.venv/bin/python main.py recognition -c config/ablation/ntu-xsub/spatial_imp.yaml --work_dir temp/amp_probe_off --num_epoch 1 --save_interval 1 --eval_interval 1 --debug --amp False`；对应 AMP 开启版只把 `--amp True` 和输出目录改为 `temp/amp_probe_on`。
- Verification: 关闭 AMP 时得到 `Top1=1.67% / Top5=8.31% / mean_loss=4.09985`；开启 AMP 时得到 `Top1=1.67% / Top5=8.33% / mean_loss=4.09908`。两者在这组最短探针上基本一致，未观察到明显异常。
- Follow-up: 后续正式 40 epoch 消融默认可以继续使用 AMP；若最终排序出现可疑翻转，再回头把 AMP 单独拿出来做更长程对照。

## 2026-04-13 - `distance partitioning` 语义审计与最小修复探针
- Context: 第一轮消融里只有 `distance_noimp` 表现异常，具体表现为训练 loss 正常下降，但验证 `Top-1` 在 `55.61 -> 41.18 -> 43.96 -> 61.85 -> 54.00 -> 46.76 -> 58.29 -> 62.81` 之间大幅震荡；用户要求做“语义级审计”，而不是停留在 adjacency shape 和参数量检查。
- Decision: 新增 [temp/audit_distance_partition.py](/home/atk/PyCharm/ST-GCN/temp/audit_distance_partition.py) 做 3 类检查：NTU 图上 `distance` 子集纯净性、3 节点链图 toy graph 下的归一化口径对比、以及 `uniform`/`distance` 的最小前向一致性测试；在 [net/utils/graph.py](/home/atk/PyCharm/ST-GCN/net/utils/graph.py) 里额外增加探针策略 `distance_subsetnorm`，用于验证“先切分、再对子集独立归一化”是否能缓解异常。
- Why: 审计结果表明，当前实现里 `distance[0]` 只含 self-loop、`distance[1]` 只含 1-hop non-self edge，且 `A_uniform == A_distance[0] + A_distance[1]` 逐元素完全守恒，`ConvTemporalGraphical` 也不存在额外数值偏置；真正最可疑的位置是 `graph.py` 里 `distance` 分支采用了“先整体归一化，再切分”的顺序。这个口径在 toy graph 上会把两个 subset 都按整体 degree 稀释掉，不像“独立关系子图”。
- Action/Command: `.venv/bin/python temp/audit_distance_partition.py`；新增 [config/ablation/ntu-xsub/distance_subsetnorm_probe.yaml](/home/atk/PyCharm/ST-GCN/config/ablation/ntu-xsub/distance_subsetnorm_probe.yaml)，并在真实环境里执行 `.venv/bin/python main.py recognition -c config/ablation/ntu-xsub/distance_subsetnorm_probe.yaml` 做 15 epoch 短程探针。
- Verification: 语义审计已排除子集脏边、support 不守恒和 tgcn 前向偏置；探针版 `distance_subsetnorm` 在 `epoch 4` 得到 `Top1=52.06% / Top5=82.77%`，首点仍低于原始 `distance_noimp` 的 `55.61% / 88.25%`；但到 `epoch 9` 时，`distance_subsetnorm` 提升到 `Top1=60.08% / Top5=88.82%`，显著高于原始 `distance_noimp` 在相同节点的 `41.18% / 74.59%`。这说明“先切分、再对子集独立归一化”明显缓解了中期验证塌陷，但没有把 `distance` 完全修好。
- Follow-up: 后续若继续查这个问题，优先围绕“subset 独立归一化 + 其余数值平衡项”继续缩小范围，而不是再回到 adjacency shape、support 或 tgcn 前向这种已排除路径。当前新增的重要理解是：`distance_subsetnorm` 把 self subset 与 hop-1 subset 都各自列归一化到 `1`，导致合并后每列总权重从原始 `distance` 的 `1` 变成 `2`；在相同卷积权重下，第一层图卷积输出绝对均值约放大到原始 `distance` 的 `~2.05x`，整网各层激活长期维持 `~1.5x-2.0x`，最终 logits 仍高约 `13%`。这解释了为什么修复版首个验证点更差但中期更稳：它同时“放大了消息量”并“显著抬高了 self 分支权重”。

## 2026-04-14 - 恢复 modern 初始重写中被压掉的重要说明
- Context: 用户指出 `f623ac0` 那批 modern 初始重写虽然完成了 Python 3.12 / PyTorch 2.x 迁移，但把一部分高价值注释、形状说明和兼容语义说明压缩得过头，后续阅读和审计成本偏高。
- Decision: 不改变任何外部接口或数值路径，只在 `main.py`、`net/`、`processor/`、`feeder/` 与 `torchlight/torchlight/io.py` 补回关键 docstring、张量形状说明、图分区语义说明，以及 checkpoint / CLI 兼容注释。
- Why: 这类说明不是“代码美化”，而是 modern 分支判断“是否仍与官方实现等价”的重要证据。尤其是 ST-GCN 的 `(N,C,T,V,M)` 数据重排、`distance/spatial` 分区语义、双流 motion 定义，以及 `torchlight.IO` 的权重过滤兼容逻辑，单靠简短函数名不足以支撑长期维护。
- Action/Command: 对照 `221c0e1` 官方基线与当前 modern 文件，恢复 `Model` / `st_gcn` / `ConvTemporalGraphical` / `Graph` / `Processor` / `REC_Processor` / feeder / `torchlight.IO` 等核心模块中的高价值说明，并执行 `python3 -m compileall main.py net processor feeder torchlight`。
- Verification: 当前相关文件均已通过 `compileall`；本次修改只涉及注释、docstring 与说明性类型语义，没有改动模型结构、参数名、配置键名或张量计算路径。
- Follow-up: 后续再做 modern 化整理时，涉及官方数学定义、张量布局、图构造、兼容加载或多阶段处理器调度的代码，默认不要把原版语义说明压缩成只剩一句短 docstring。

## 2026-04-15 - `distance partitioning` 补偿假设实验设计
- Context: 用户与外部讨论后，将第二轮 `distance partitioning` 实验目标进一步收缩为验证“原版 subset balance 偏置是否存在，以及 `edge importance weighting` 是否在补偿这种偏置”。用户明确要求先把详细实验设计写成独立文档，避免后续边实现边漂移。
- Decision: 在 [doc/2026-04-15_ST-GCN distance partitioning 补偿假设实验设计.md](/home/atk/PyCharm/ST-GCN/doc/2026-04-15_ST-GCN%20distance%20partitioning%20补偿假设实验设计.md) 固定这轮设计：第一阶段 4 个 `20 epoch`（`distance_imp`、`distance_subsetnorm_rescaled_noimp`、`distance_subsetnorm_rescaled_imp`、`spatial_subsetnorm_rescaled_noimp`），第二阶段 2 个 `40 epoch`（固定 `distance_imp`，另一个按第一阶段结果在 `distance_subsetnorm_rescaled_imp` 与 `spatial_subsetnorm_rescaled_noimp` 中二选一）。同时固定 `A_eff = A ⊙ M` 的轻量统计口径。
- Why: 前一轮 `distance_subsetnorm` 探针已经说明，单纯“先切分再归一化”会明显改变总消息量与 self / neighbor 的相对强度。如果不先钉死 `subsetnorm_rescaled` 的定义，后续任何结果都不好解释。
- Action/Command: 文档中已固定 `subsetnorm_rescaled` 的定义为“对子集分别 `normalize_digraph` 后，统一乘以 `1/K`”，其中 `distance` 用 `K=2`，`spatial` 用 `K=3`；并明确这轮不做裸 `subsetnorm`、不做 `uniform` 新变体、不做多 seed，也不优先做热图。
- Verification: 当前这只是设计冻结，不涉及代码实现和数值验证；但后续实现必须严格对齐该文档中的实验名、训练协议、判据和统计量。2026-04-15 同时已在训练入口补入显式 `--seed`，默认固定为 `49`；该值用于后续新实验的正式固定种子，不反向覆盖旧实验配置或旧结果的历史语义。
- Follow-up: 下一步实现时，优先围绕 `graph.py` 新增 `distance_subsetnorm_rescaled` / `spatial_subsetnorm_rescaled`，再补对应配置与 `A_eff` 统计脚本，不要先随手开跑。

## 2026-04-15 - `distance partitioning` 补偿假设实验结果与权重分析
- Context: 六组补偿假设实验已全部完成，用户要求统一整理实验可视化、`A_eff` 权重分析，并写成正式实验报告留档。
- Decision: 新增 [tools/plot_distance_compensation_results.py](/home/atk/PyCharm/ST-GCN/tools/plot_distance_compensation_results.py)，统一读取 6 组日志与两份 `effective_adjacency.csv`，输出两张 SciencePlots 风格图：一张结果总图（`Top-1 / Top-5 / validation loss / Best-Final Top-1`），一张权重分析图（两组 `imp` 模型的 `mask_mean` 与 `A_eff` 强度对比）。正式报告写入 [doc/2026-04-15_ST-GCN distance partitioning 补偿假设实验报告.md](/home/atk/PyCharm/ST-GCN/doc/2026-04-15_ST-GCN%20distance%20partitioning%20补偿假设实验报告.md)。
- Why: 这轮实验的核心不只是“谁更高”，还要同时回答三个层面：`importance` 是否显著补偿原版 `distance` 异常、`subsetnorm_rescaled` 是否存在独立净收益，以及这种收益是否能从训练后有效邻接 `A_eff = A ⊙ M` 的子集强度分布上读出来。把结果图和权重图拆开后，阅读负担更小，也更接近论文式叙述顺序。
- Action/Command: 使用 `.venv/bin/python tools/plot_distance_compensation_results.py` 实际导出 [ntu_xsub_distance_compensation_results.png](/home/atk/PyCharm/ST-GCN/work_dir/figures/ntu_xsub_distance_compensation_results.png)、[ntu_xsub_distance_compensation_results.pdf](/home/atk/PyCharm/ST-GCN/work_dir/figures/ntu_xsub_distance_compensation_results.pdf)、[ntu_xsub_distance_compensation_weights.png](/home/atk/PyCharm/ST-GCN/work_dir/figures/ntu_xsub_distance_compensation_weights.png)、[ntu_xsub_distance_compensation_weights.pdf](/home/atk/PyCharm/ST-GCN/work_dir/figures/ntu_xsub_distance_compensation_weights.pdf)；`A_eff` 统计通过 [tools/export_effective_adjacency_stats.py](/home/atk/PyCharm/ST-GCN/tools/export_effective_adjacency_stats.py) 的 `--work-dir` 支持读取 `distance_imp_e40` 与 `distance_subsetnorm_rescaled_imp_e40` 的 checkpoint。
- Verification: 六组实验的关键数值结论为：阶段一 `A3(distance_subsetnorm_rescaled_imp)` 全程略优于 `A1(distance_imp)`，阶段二 `B2(distance_subsetnorm_rescaled_imp_e40)` 最终达到 `Top1=77.79% / Top5=96.26%`，相对 `B1(distance_imp_e40)` 的 `76.10% / 95.40%` 提升 `+1.69 / +0.86`。`A_eff` 权重统计显示，原版 `distance_imp_e40` 的 `self/neighbor a_eff_colsum_mean` 比值均值仅 `0.778`，整体仍然 neighbor-dominant；而 `distance_subsetnorm_rescaled_imp_e40` 的对应均值达到 `1.226`，已转向 self-balanced / self-favoring。两组模型的 `mask_mean` 比值均值都接近 `1`（约 `1.017` vs `1.009`），说明真正关键差异不在 `M` 平均值本身，而在 `A ⊙ M` 的有效子集强度分布。
- Follow-up: 后续若继续沿这条线推进，优先把 `distance_subsetnorm_rescaled` 视为更合理的 `distance partitioning` 实现口径；`spatial_subsetnorm_rescaled_imp` 可作为补充验证，但不是当前最紧急工作。

## 2026-04-16 - 语义审计报告补充直观对比图
- Context: 用户指出 [doc/2026-04-13_ST-GCN distance partitioning 语义审计报告.md](/home/atk/PyCharm/ST-GCN/doc/2026-04-13_ST-GCN%20distance%20partitioning%20语义审计报告.md) 只看实验汇总图不够直观，希望把真正支撑结论的比较用图直接补进报告。
- Decision: 新增 [tools/plot_distance_semantic_audit.py](/home/atk/PyCharm/ST-GCN/tools/plot_distance_semantic_audit.py)，为语义审计单独输出 5 张图：toy graph 矩阵热图、`distance_noimp` vs `distance_subsetnorm_probe` 的短程验证曲线对照、NTU 图上 current `distance` / `distance_subsetnorm` 的 self-neighbor-merged 列和分布对比、`uniform`/`distance` 的最小前向一致性对比，以及 `distance_subsetnorm` 的受控尺度放大图；同时把图片直接嵌入报告对应章节，而不是只在末尾放产物链接。
- Why: 语义审计的核心判断不是“最终谁更高”，而是“每条结论究竟由哪组受控证据支撑”。因此仅补一张短程 probe 图仍不够，需要让 toy graph、完整基线+短程 probe、列和平衡、最小前向一致性、以及尺度放大分别对应报告中的不同结论段。
- Action/Command: `.venv/bin/python -m compileall tools/plot_distance_semantic_audit.py`；`.venv/bin/python tools/plot_distance_semantic_audit.py`。
- Verification: 已成功导出 [ntu_xsub_distance_semantic_toy_matrices.png](/home/atk/PyCharm/ST-GCN/work_dir/figures/ntu_xsub_distance_semantic_toy_matrices.png)、[ntu_xsub_distance_semantic_probe_curves.png](/home/atk/PyCharm/ST-GCN/work_dir/figures/ntu_xsub_distance_semantic_probe_curves.png)、[ntu_xsub_distance_semantic_column_balance.png](/home/atk/PyCharm/ST-GCN/work_dir/figures/ntu_xsub_distance_semantic_column_balance.png)、[ntu_xsub_distance_semantic_forward_equivalence.png](/home/atk/PyCharm/ST-GCN/work_dir/figures/ntu_xsub_distance_semantic_forward_equivalence.png)、[ntu_xsub_distance_semantic_scale_amplification.png](/home/atk/PyCharm/ST-GCN/work_dir/figures/ntu_xsub_distance_semantic_scale_amplification.png) 及对应 PDF；短程 probe 图现已保留原始 `distance_noimp` 的完整 `40 epoch` 轨迹，并用灰色区显式标出“probe 已停止而基线继续”的后半段，差值子图只在公共评估点上比较。
- Follow-up: 后续若再写“结构审计 / 语义审计 / 补偿假设”类报告，优先为每个核心论点单独出小型比较图，并直接插在论点段落后；如果图内需要放说明文字，先给 y 轴留 padding，再决定文本位置，避免把范围说明压到曲线顶部。当前仓库文档渲染里，图像路径应优先写成 `./work_dir/...`，不要再用 `../work_dir/...`。

## 2026-04-16 - 补偿实验报告按阶段拆图
- Context: 用户指出 [doc/2026-04-15_ST-GCN distance partitioning 补偿假设实验设计.md](/home/atk/PyCharm/ST-GCN/doc/2026-04-15_ST-GCN%20distance%20partitioning%20补偿假设实验设计.md) 和 [doc/2026-04-15_ST-GCN distance partitioning 补偿假设实验报告.md](/home/atk/PyCharm/ST-GCN/doc/2026-04-15_ST-GCN%20distance%20partitioning%20补偿假设实验报告.md) 只有总图和权重图，不足以支撑“阶段一筛查 / 阶段二确认 / 与第一轮桥接”这些分阶段分析。
- Decision: 在 [tools/plot_distance_compensation_results.py](/home/atk/PyCharm/ST-GCN/tools/plot_distance_compensation_results.py) 基础上新增 3 张分工更明确的图：`ntu_xsub_distance_compensation_stage1.*`、`ntu_xsub_distance_compensation_stage2.*`、`ntu_xsub_distance_compensation_baseline_bridge.*`；并把它们分别嵌入阶段一、阶段二和基线对照分析段落，同时在设计文档增加“对应分析图规划”小节。
- Why: 补偿实验总图适合鸟瞰，但不适合承担所有论点。把“20 epoch 筛查”“40 epoch head-to-head”“与第一轮原始基线桥接”拆开后，每张图只回答一个问题，阅读负担更低，分析也更不容易串味。
- Action/Command: `.venv/bin/python -m compileall tools/plot_distance_compensation_results.py`；`.venv/bin/python tools/plot_distance_compensation_results.py`。
- Verification: 已成功导出 [ntu_xsub_distance_compensation_stage1.png](/home/atk/PyCharm/ST-GCN/work_dir/figures/ntu_xsub_distance_compensation_stage1.png)、[ntu_xsub_distance_compensation_stage2.png](/home/atk/PyCharm/ST-GCN/work_dir/figures/ntu_xsub_distance_compensation_stage2.png)、[ntu_xsub_distance_compensation_baseline_bridge.png](/home/atk/PyCharm/ST-GCN/work_dir/figures/ntu_xsub_distance_compensation_baseline_bridge.png) 及对应 PDF，并在补偿实验报告中按章节插图。
- Follow-up: 以后这类多阶段实验报告，默认至少拆成“阶段筛查图 + 长程确认图 + 基线桥接图 + 机制图/权重图”；总图保留，但不再让它独自承担全部分析。

## 2026-04-16 - 作图标题区必须显式留白
- Context: 用户指出 `ntu_xsub_distance_semantic_forward_equivalence.png` 的总标题与子图标题几乎重叠，并进一步提醒这很可能不是单张图问题，而是多个脚本共用的版式缺陷。
- Decision: 在 [tools/plot_distance_semantic_audit.py](/home/atk/PyCharm/ST-GCN/tools/plot_distance_semantic_audit.py)、[tools/plot_distance_compensation_results.py](/home/atk/PyCharm/ST-GCN/tools/plot_distance_compensation_results.py)、[tools/plot_training_comparison.py](/home/atk/PyCharm/ST-GCN/tools/plot_training_comparison.py)、[tools/plot_ablation_results.py](/home/atk/PyCharm/ST-GCN/tools/plot_ablation_results.py) 统一新增 `finalize_figure(...)`，先把布局矩形压到 `top≈0.90` 再放 `suptitle`，不再通过 `y=1.01/1.02` 这类硬顶方式碰运气。
- Why: `constrained_layout=True` 与 `fig.suptitle(..., y=1.01)` 的组合很容易让总标题和子图标题互相争顶边距；单纯上移或下移标题都不稳，只有先给图面预留独立标题区才可靠。
- Action/Command: `.venv/bin/python -m compileall tools/plot_distance_semantic_audit.py tools/plot_distance_compensation_results.py tools/plot_training_comparison.py tools/plot_ablation_results.py`；分别重跑 4 个作图脚本。
- Verification: 当前相关图像产物均已重新生成，包括语义审计图、补偿实验图、正式训练对比图和第一轮消融图；这轮修改的目标不是改内容，而是统一修复总标题与子图标题的垂直间距。
- Follow-up: 后续新增作图脚本时，默认直接带 `finalize_figure(...)` 这一层；不要再把“总标题贴太近”当成单张图的小毛病处理。
